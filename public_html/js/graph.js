class Graph {
  ...
  drawArea() {
    const radiiToDraw =
      this.multipleRadii.length > 0 ? this.multipleRadii : [this.currentR];
    ...
    radiiToDraw.forEach((radius, index) => {
      ...
      this.ctx.strokeStyle = colors[index % colors.length].replace(
        "0.25",
        "0.8",
      );
  ...
  drawPoints() {
    ...
    this.points.forEach((point) => {
      if (!point.r || activeRadii.includes(point.r)) {
